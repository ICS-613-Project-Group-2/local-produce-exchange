...
export default function Dashboard() {
  ...
  const [expiringSoon, setExpiringSoon] = useState<ListingResponse[]>([]);
  ...
  useEffect(() => {
    ...
    async function loadDashboard() {
      try {
        ...
        // Date.now() is safe here since this runs inside an effect callback,
        // not during render.
        const now = Date.now();
        const active = listingsData.filter(
          (l) => l.status === "available" || l.status === "expiring-soon"
        );
        setExpiringSoon(
          active.filter((l) => {
