...
export default function CommunityDetail() {
  ...
  useEffect(() => {
    // eslint-disable-next-line react-hooks/set-state-in-effect -- false positive: setState calls in loadCommunity happen inside try/catch/finally after an await (see facebook/react#34905)
    loadCommunity();
...
function PostsTab({ communityId }: { communityId: number }) {
  ...
  useEffect(() => {
    // eslint-disable-next-line react-hooks/set-state-in-effect -- false positive: setState calls in loadPosts happen inside try/catch/finally after an await (see facebook/react#34905)
    loadPosts();
    // eslint-disable-next-line react-hooks/exhaustive-deps -- loadPosts is redefined each render; communityId is the real trigger
  }, [communityId]);
...
function MembersTab({ communityId }: { communityId: number }) {
  ...
  useEffect(() => {
    // eslint-disable-next-line react-hooks/set-state-in-effect -- false positive: setState calls in loadMembers happen inside try/catch/finally after an await (see facebook/react#34905)
    loadMembers();
